...
function M.extend_lspconfig()
	...
	local lsp_cmds = vim.api.nvim_create_augroup("lsp_default_attach", { clear = true })
	vim.api.nvim_create_autocmd("LspAttach", {
		group = lsp_cmds,
		desc = "lsp-default on_attach",
...
function M.setup(name, opts)
	...
	if not ok then
		local msg = "[lsp-default] Failed to setup %s.\n\n"
...
function M.set_global_commands()
	...
	command("LspDefaultWorkspaceAdd", "lua vim.lsp.buf.add_workspace_folder()", {})

	command("LspDefaultWorkspaceList", "lua vim.notify(vim.inspect(vim.lsp.buf.list_workspace_folders()))", {})
end
...
function s.set_buf_commands(bufnr)
	...
	local format = function(input)
		if #input.fargs > 2 then
			vim.notify("Too many arguments for LspDefaultFormat", vim.log.levels.ERROR)
			return
		end
	...
	bufcmd(bufnr, "LspDefaultFormat", format, { range = true, bang = true, nargs = "*" })

	bufcmd(bufnr, "LspDefaultWorkspaceRemove", "lua vim.lsp.buf.remove_workspace_folder()", {})
end
...
function M.setup_installer()
	...
	local installer = require("lsp-default.installer")
	local config = require("lsp-default.settings").get()
...
function s.set_capabilities(current)
	if state.capabilities == nil then
		...
		state.capabilities = vim.tbl_deep_extend("force", base, cmp_default_capabilities, current or {})
...
function s.apply_global_config(config, user_config, defaults)
	...
	for key, val in pairs(new_config) do
		...
		elseif key == "on_new_config" and config[key] and config[key] ~= new_config[key] then
			local cb = config[key]
