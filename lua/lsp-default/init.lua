...
local Server = require("lsp-default.server")

if vim.fn.has("nvim-0.8") == 1 then
	Server.extend_lspconfig()
else
	local msg = "[lsp-default] You need Neovim v0.8 or greater to use lsp-default v2.\n"
		.. "Use the v1.x branch if you need compatibility with Neovim v0.7 or lower."
	vim.notify(msg, vim.log.levels.WARN)
end

function M.cmp_action()
	return require("lsp-default.cmp-mapping")
end
...
function M.setup()
	...
	require("lsp-default.setup").apply(s.args)
end

function M.preset(opts)
	require("lsp-default.settings").preset(opts)
...
function M.build_options(name, opts)
	...
	return vim.tbl_deep_extend("force", defaults, Server.default_config or {}, opts or {})
end
...
function M.use(servers, opts)
	...
	for _, name in ipairs(servers) do
		local config = vim.tbl_deep_extend("force", s.lsp_project_configs[name] or {}, opts or {})
...
function M.extend_cmp(opts)
	require("lsp-default.cmp").extend(opts)
end
...
function M.new_server(opts)
	...
	require("lsp-default.client").setup(config)
end

function M.format_on_save(opts)
	return require("lsp-default.format").format_on_save(opts)
end

function M.format_mapping(...)
	return require("lsp-default.format").format_mapping(...)
end

function M.buffer_autoformat(...)
	return require("lsp-default.format").buffer_autoformat(...)
end

function M.async_autoformat(...)
	return require("lsp-default.format").async_autoformat(...)
end

M.dir = {}

function M.dir.find_all(list)
	return require("lsp-default.dir").find_all(list)
end

function M.dir.find_first(list)
	return require("lsp-default.dir").find_first(list)
end

M.omnifunc = {}

function M.omnifunc.setup(opts)
	require("lsp-default.omnifunc").setup(opts)
end
...
function M.set_preferences(opts)
	if type(opts) == "table" then
		require("lsp-default.settings").set(opts)
	end
end
...
function M.defaults.cmp_mappings(opts)
	local cmp_setup = require("lsp-default.cmp")
	local config = vim.tbl_deep_extend("force", cmp_setup.basic_mappings(), cmp_setup.extra_mappings())
...
function M.defaults.cmp_sources(opts)
	local config = require("lsp-default.cmp").sources()
...
function M.extend_lspconfig(opts)
	...
	if not ok then
		local msg = "[lsp-default] Could not find the module lspconfig. Please make sure 'nvim-lspconfig' is installed."
