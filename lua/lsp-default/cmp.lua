...
function M.extend(opts)
	local defaults = require("lsp-default.preset").minimal().manage_nvim_cmp
...
function M.apply(cmp_opts, user_config)
	if not ok_cmp then
		local msg =
			"[lsp-default] Could not find nvim-cmp. Please install nvim-cmp or set the option `manage_nvim_cmp` to false."
	...
	elseif vim.tbl_isempty(cmp_config.mapping) == false and vim.tbl_isempty(config.mapping) == false then
	...
	if type(opts.documentation) == "table" then
		...
		config.window.documentation = s.merge(config.window.documentation, opts.documentation)
