...
local format_group = "lsp_default_format"
...
function M.format_on_save(opts)
	...
	local setup_id = augroup("lsp_default_format_setup", { clear = true })
	...
	local filetype_setup = function(event)
		...
		local apply_format = function()
			local autoformat = vim.b.lsp_default_enable_autoformat
...
function M.buffer_autoformat(client, bufnr, format_opts)
	...
	if vim.b.lsp_default_enable_autoformat == nil then
		vim.b.lsp_default_enable_autoformat = 1
	end
...
function M.format_mapping(key, opts)
	...
	local autocmd = vim.api.nvim_create_autocmd
	local augroup = vim.api.nvim_create_augroup
	local format_id = augroup("lsp_default_format_mapping", { clear = true })
...
function M.check(server)
	...
	if client == nil then
		local msg = "[lsp-default] %s is not active"
		vim.notify(msg:format(server), vim.log.levels.WARN)
		return
	end

	if vim.lsp.buf_is_attached(buffer, client.id) == false then
		local msg = "[lsp-default] %s is not active in the current buffer"
		vim.notify(msg:format(server), vim.log.levels.WARN)
		return
	end

	if client.supports_method("textDocument/formatting") == false then
		local msg = "[lsp-default] %s does not support textDocument/formatting method"
		vim.notify(msg:format(server), vim.log.levels.WARN)
		return
	end

	local msg = "[lsp-default] %s has formatting capabilities"
...
function s.request_format(client_id, buffer, format_opts, timeout)
	if vim.b.lsp_default_format_progress == 1 then
		local msg = "[lsp-default] A formatting request is already in progress."
	...
	if vim.b.lsp_default_changedtick == vim.b.changedtick then
		return
	end
	...
	vim.b.lsp_default_changedtick = vim.b.changedtick
	vim.b.lsp_default_format_progress = 1
...
function s.format_handler(err, result, ctx)
	...
	local fmt_var = "lsp_default_format_progress"
	local tick_var = "lsp_default_changedtick"
	...
	if err ~= nil then
		vim.notify("[lsp-default] Format request failed.", vim.log.levels.WARN)
		buf_set(buffer, fmt_var, 0)
		return
	end

	if result == nil then
		local msg = "[lsp-default] %s could not format file."
	...
	if current_changedtick ~= buf_changedtick then
		local msg = "[lsp-default] Format canceled. Buffer was modified after request."
		vim.notify(msg, vim.log.levels.WARN)
		buf_set(buffer, fmt_var, 0)
		return
	end
...
function s.format_cleanup(buffer, client_name)
	...
	buf_set(buffer, "lsp_default_format_progress", 0)

	local msg = "[lsp-default] Format request timeout. %s is taking too long to respond."

	vim.notify(msg:format(client_name), vim.log.levels.WARN)

	local changedtick = buf_get(buffer, "lsp_default_changedtick")
	local current_changedtick = buf_get(buffer, "changedtick")

	if changedtick == current_changedtick then
		buf_set(buffer, "lsp_default_changedtick", changedtick - 1)
	end
end
