...
function M.apply(args)
	...
	local user_settings = require("lsp-default.settings").get()
	local cmp_txt = vim.api.nvim_get_runtime_file("doc/cmp.txt", 1) or {}

	if user_settings.manage_nvim_cmp and #cmp_txt > 0 then
		require("lsp-default.cmp").apply(args.cmp_opts, user_settings.manage_nvim_cmp)
	end
	...
	if use_local then
		installer = require("lsp-default.installer")
...
function s.setup_ui(opts)
	...
	vim.lsp.handlers["textDocument/signatureHelp"] =
		vim.lsp.with(vim.lsp.handlers.signature_help, { border = opts.border })
